...
@level2
        IF_MORE_THAN ENEMIES_KILLED_LOW, #25, @level3
...
@level3
        IF_MORE_THAN ENEMIES_KILLED_LOW, #70, @level4
...
@level4
        IF_MORE_THAN ENEMIES_KILLED_LOW, #100, @level5
        lda CURRENT_LEVEL
        cmp #4
        beq @run_level4
...
@level5
        IF_MORE_THAN ENEMIES_KILLED_LOW, #135, @level6
        lda CURRENT_LEVEL
        cmp #5
        beq @run_level5
...
@run_level5
        lda #%11100111 ;Turn enemy 4 off and 1 and bullet on
        sta SPRITE_ENABLED_ADDRESS
        ldx #0
        stx ENEMY_3_VARIATION
        stx ENEMY_2_VARIATION
...
@level6
        IF_MORE_THAN ENEMIES_KILLED_LOW, #175, @level7
        lda CURRENT_LEVEL
        cmp #6
        beq @run_level6
...
@level7
        IF_MORE_THAN ENEMIES_KILLED_LOW, #220, @level8
        lda CURRENT_LEVEL
        cmp #7
        beq @run_level7
...
@level8
        IF_MORE_THAN ENEMIES_KILLED_LOW, #255, @level9
        lda CURRENT_LEVEL
        cmp #8
        beq @run_level8
