...
defm CHECK_IF_ENEMY_HAS_COLLIDED_WITH_BULLET ; (ENEMY_HIT, ENEMY X ADDRESS, FRAME )
        ...
        ldx /2 ; Load the x register with X position
        ldy /2 + 1 ; load the y register with the next memory posiiton, which is the Y position
        
        jsr CHECK_PLAYER_BULLET_HIT_ENEMY
        cmp #TRUE
        beq @has_collided
...
@has_collided        
        ...
        lda #EXPLOSION_F1_SPRITE_VALUE ; set robot frame to explosion
        sta /3
